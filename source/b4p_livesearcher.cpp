...
void LiveSearcher::Search(){
    while( true ){
        ...
        QNetworkRequest rq( QUrl( "http://live.nicovideo.jp/search?track=&sort=recent&date=&keyword=%E3%83%90%E3%82%A4%E3%82%AA4&filter=+:hidecomonly:&kind=tags" ) );
        ...
        QRegExp reg_html( "<div class=\"result_list\">(.*)<\/div>\r\n\t\t<div class=\"notice_msg\" >" );
        reg_html.indexIn( html );
        html = reg_html.cap( 1 );
        reg_html.setPattern( "v=(lv[0-9]+)" );
        int pos = 0;
        while( ( pos = reg_html.indexIn( html, pos ) ) != -1 ){
            QString URL_GETLIVEINFOSTATUS = "http://watch.live.nicovideo.jp/api/getplayerstatus?v=";
            URL_GETLIVEINFOSTATUS.append( reg_html.cap( 1 ) );
            ...
            pos += reg_html.matchedLength();
            pos = reg_html.indexIn( html, pos );
            pos += reg_html.matchedLength();
        }
        QThread::sleep( 60 );
    }
